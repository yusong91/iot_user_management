<div class="avatar-wrapper">
    ...
    <div class="text-center">
        <div class="avatar-preview">
            <img class="avatar rounded-circle img-thumbnail img-responsive mt-5 mb-4" width="150" src="{{ $edit ? $user->present()->avatar : url('assets/img/profile.png') }}">
            <h5 class="text-muted">{{ $user->present()->nameOrEmail }}</h5>
        </div>

        <div id="change-picture"
             class="btn btn-outline-secondary btn-block mt-5" data-toggle="modal" data-target="#choose-modal"><i class="fa fa-camera"></i>@lang('Change Photo')
        </div>
            ...
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-4 avatar-source" id="no-photo"
                         data-url="{{ $updateUrl }}"> <img src="{{ url('assets/img/profile.png') }}" class="rounded-circle img-thumbnail img-responsive"><p class="mt-3">@lang('No Photo')</p>
                    </div>
                    ...
                    @if ($edit)
                        <div class="col-md-4 avatar-source source-external"
                             data-url="{{ $updateUrl }}"> <img src="{{ $user->gravatar() }}" class="rounded-circle img-thumbnail img-responsive"> <p class="mt-3">@lang('Gravatar')</p>
                        </div>
                    @endif
                </div>
                    ...
                    @foreach ($socialLogins->chunk(3) as $logins)
                        ...
                        <div class="row">
                            @foreach($logins as $login)
                                <div class="col-md-4 avatar-source source-external"
                                     data-url="{{ $updateUrl }}"> <img src="{{ $login->avatar }}" class="rounded-circle img-thumbnail img-responsive" style="width: 120px;"><p>{{ ucfirst($login->provider) }}</p>
                                </div>
                            @endforeach
                        </div>
                    @endforeach
