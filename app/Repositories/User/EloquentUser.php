...
class EloquentUser implements UserRepository
{
    ...
    public function paginate($perPage, $search = null, $status = null)
    {
        ...
        if($role_id == 1)
        {
            $query =  User::query()->where('role_id', 2)->where('parent_id', 0);
            //$query =  User::query();
        }
        elseif ($role_id == 2)
        {   
            ...
            $query = User::query()->where('family', $family)->where('role_id', '!=', 1)->orWhere('id', auth()->user()->id);
        ...
        $result = $query->orderBy('role_id', 'asc') //desc
            ->paginate($perPage);
    ...
    {
        return User::leftJoin('sessions', 'users.id', '=', 'sessions.user_id')->select('users.*')->where('sessions.id', $sessionId)->first();
    }
    ...
    {
        return User::orderBy('created_at', 'DESC')->limit($count)->get();
    }

    public function countOfNewUsersPerMonth(Carbon $from, Carbon $to)
    {
        $result = User::whereBetween('created_at', [$from, $to])->orderBy('created_at')->get(['created_at'])->groupBy(function ($user) {
    ...
    {
        return DB::table('social_logins')->where('user_id', $userId)->get();
    }
